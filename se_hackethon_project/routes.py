...
from se_hackethon_project.models import User, Post, Feedback, Accuracy
from flask_login import login_user, current_user, logout_user, login_required
from flask_mail import Message
from requests import get

tips = ["Keep your shoulders back. Do not hunch over your screen. It can cause shoulder and upper back stiffness", "Sit up with your back straight", "Bending your shoulders towards one side increases joint pain", "Go back from the screen, You tend to come too close!!!", "Do not slouch", "Keep calm & sit straight!", "Do not bend your neck", "Your neck may get twisted, be careful!"]
tipsLength = len(tips);
...
@login_required
def analyze_posture():
    tip1 = tips[random.randint(0,tipsLength-1)]
    tip2 = tips[random.randint(0,tipsLength-1)]
    tip3 = tips[random.randint(0,tipsLength-1)]
    f1 = 0
    f2 = 0
    f3 = 0
    f4 = 0
    ...
    analyzeSes = Accuracy(f1=f1,f2=f2,f3=f3,f4=f4,results=current_user)
    db.session.add(analyzeSes)
    db.session.commit()
    # sessions = Accuracy.query.filter_by(results=current_user).all()
    # print("#################################")
    # print(sessions)
    # dataset = []
    # for pos in sessions:
    #     dataset.append([pos.f1,pos.f2,pos.f3,pos.f4,pos.date_posted])
    # print(dataset)
    flash("Your session added to db", 'success')
    return render_template('graph.html', x1=f1, x2=f2, x3=f3, x4=f4, tip1=tip1, tip2=tip2,tip3 = tip3)
...
def analyze_sessions():
    ...
    f3 = format(f3, ".2f")
    f4 = format(f4, ".2f")
    return render_template('average.html', x1=f1, x2=f2, x3=f3, x4=f4, tip1=tip1, tip2=tip2, tip3 = tip3)
